/*
  ...
  1. New Tables
    - `podcasts`
      - For storing podcast metadata
      - Tracks plays and likes
...
CREATE OR REPLACE FUNCTION increment_podcast_likes()
RETURNS TRIGGER AS $$
BEGIN
  UPDATE podcasts
...
CREATE OR REPLACE FUNCTION decrement_podcast_likes()
RETURNS TRIGGER AS $$
BEGIN
  UPDATE podcasts
  SET likes_count = likes_count - 1
...
CREATE TRIGGER increment_podcast_likes_trigger
  AFTER INSERT ON podcast_likes
  FOR EACH ROW
  EXECUTE FUNCTION increment_podcast_likes();

CREATE TRIGGER decrement_podcast_likes_trigger
  AFTER DELETE ON podcast_likes
  FOR EACH ROW
  EXECUTE FUNCTION decrement_podcast_likes();

-- Create function to increment play count
CREATE OR REPLACE FUNCTION increment_podcast_plays()
RETURNS TRIGGER AS $$
BEGIN
  UPDATE podcasts
